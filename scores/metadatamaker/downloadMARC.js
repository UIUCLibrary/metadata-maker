...
function fillISMN(record,head,fieldFunc,subfieldFunc) {
	...
	if (checkExists(record.ismn)) {
		var ismn = fieldFunc(tag,'2',' ',[subfieldFunc('a',record.ismn)]);

		return returnSingleEntry(tag,ismn,head);
	}
	else {
		return head !== null ? ['',''] : '';
	}
...
function fillUniformTitle(record,head,fieldFunc,subfieldFunc) {
	...
	if (checkExists(record.uniform_title)) {
		...
		var uniform_title = fieldFunc(tag,'1',title_ind2,[subfieldFunc('a',record.uniform_title)]);

		return returnSingleEntry(tag,uniform_title,head);
	}
	else {
		return head !== null ? ['',''] : '';
	}
...
function fillFormattedContentNote(record,head,fieldFunc,subfieldFunc) {
	...
	if (checkExists(record.formatted_contents_note)) {
		var formatted_contents_note = fieldFunc(tag,'0',' ',[subfieldFunc('a',record.formatted_contents_note)]);

		return returnSingleEntry(tag,formatted_contents_note,head);
	}
	else {
		return head !== null ? ['',''] : '';
	}
...
function downloadXML(record,institution_info) {
	...
	text += fillISMN(record,null,createMARCXMLField,createMARCXMLSubfield);
	text += fillAuthor(record,null,createMARCXMLField,createMARCXMLSubfield);
	text += fillUniformTitle(record,null,createMARCXMLField,createMARCXMLSubfield);
	...
	text += fillFormattedContentNote(record,null,createMARCXMLField,createMARCXMLSubfield);
